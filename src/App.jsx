...
import posterImage from './Last-Minute-Games.png';
    ...
    return (
        <div className="min-h-screen bg-neutral-950 text-neutral-100 font-sans selection:bg-amber-500/30">
            ...
            <div className="w-full flex justify-center bg-[#2d3a54] pt-8 pb-12 px-4 shadow-2xl border-b border-white/10 relative overflow-hidden">
                <div className="absolute inset-0 bg-cover bg-center opacity-30 blur-3xl" style={{ backgroundImage: `url(${posterImage})` }}></div>
                <div className="relative z-10 max-w-2xl w-full">
                    <img src={posterImage} alt="Last Minute Board Games New Year's Eve Party" className="w-full h-auto rounded-xl shadow-2xl ring-1 ring-white/10" />
                </div>
            </div>
                    ...
                    <div className="flex gap-4 md:gap-8 justify-center mb-12">
                        {[
                            ...
                            <div key={unit.label} className="flex flex-col items-center">
                                <span className="text-3xl md:text-5xl font-mono font-bold">{String(unit.val).padStart(2, '0')}</span>
                                <span className="text-[10px] uppercase tracking-widest text-neutral-500 font-bold mt-1">{unit.label}</span>
                            </div>
                        ))}
                    </div>

                    <button onClick={() => document.getElementById('rsvp-section').scrollIntoView({ behavior: 'smooth' })}
                        className="group flex items-center gap-2 bg-white text-black px-8 py-4 rounded-full font-bold text-lg hover:bg-neutral-200 transition-all transform hover:scale-105"
                    >
                        Count Me In
            ...
            <section id="rsvp-section" className="bg-neutral-900/50 border-y border-neutral-800 py-24">
                <div className="max-w-5xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-start">
                    ...
                    {/* Guest Count Side (Privacy Focused) */}
                    <div className="bg-neutral-900 rounded-[2.5rem] p-8 border border-neutral-800 shadow-2xl flex flex-col items-center text-center justify-center min-h-[400px]">
                        <div className="mb-8 p-6 rounded-full bg-neutral-800/50 relative">
                            <Users size={64} className="text-neutral-500" />
                            <div className="absolute -top-2 -right-2 bg-amber-500 text-black font-bold rounded-full w-8 h-8 flex items-center justify-center animate-bounce">
                                +
                            </div>
                        </div>
